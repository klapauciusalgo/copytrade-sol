...
import { DecisionEngine, ExecutionEngine } from '@copytrade/execution';
...
async function syncTargets() {
  ...
  // Start watching all unique targets
...
const worker = new Worker('executionQueue', async (job) => {
  ...
  try {
    ...
    if (!tx) {
      ...
      return; // Could throw to let BullMQ retry
    }
    ...
    // Enrich Token Info for Notifications
    ...
    // 4. Execution Engine (For each user copying)
    for (const req of executionRequests) {
      ...
      if (req.mode === 'MONITOR') {
        ...
        await bot.telegram.sendMessage(userWallet.user.telegramId, msg, { parse_mode: 'HTML' }).catch(() => {});
        continue;
      }
      ...
      try {
        ...
        console.log(`[Worker] Executed successfully for Telegram ID ${userWallet.user.telegramId}`);
        ...
        await bot.telegram.sendMessage(userWallet.user.telegramId, message, { parse_mode: 'HTML' }).catch(() => {});

      } catch (err: any) {
        console.error(`[Worker] Failed to execute for ${userWallet.user.telegramId}: ${err.message}`);
        // Notify user of failure
        await bot.telegram.sendMessage(userWallet.user.telegramId, `❌ <b>Trade Failed</b>\nTarget: <code>${event.wallet}</code>\nReason: ${err.message}`, { parse_mode: 'HTML' }).catch(() => {});
      }
    }
